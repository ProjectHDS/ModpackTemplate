...
f_list = f.readlines()
for i, line in enumerate(f_list):
    if (line.lstrip().startswith("echo Starting TemplateModpack ")):
        f_list[i] = "echo Starting TemplateModpack build " + version + " server...\n"
    if (line.lstrip().startswith("title \"TemplateModpack\"")):
        f_list[i] = "title \"TemplateModpack build " + version + " Server\"\n"
f.close()
